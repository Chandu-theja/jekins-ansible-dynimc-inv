...
pipeline {
  ...
  stages {
    ...
    // Using Terraform to create the Servers
    stage('CreateServers') {
      steps {
        script {
    ...
    stage('RunPlaybook') {
      steps {
        sh "whoami"
        // List the dynamic inventory just for verification
        sh "ansible-inventory --graph -i inventory/aws_ec2.yaml"
        // Run playbook using dynamic inventory & limit execution only for tomcat servers
